...
# For some reason, citra isn't always happy when you try to run it for the first
# time, so we build a simple dummy program to make sure it's okay
COPY ./docker/dummy /tmp/dummy
WORKDIR /tmp/dummy
RUN make
...
COPY --from=devkitarm /tmp/dummy/dummy.3dsx /tmp/
...
RUN xvfb-run citra --appimage-extract-and-run /tmp/dummy.3dsx; \
    rm -f /tmp/dummy.3dsx
